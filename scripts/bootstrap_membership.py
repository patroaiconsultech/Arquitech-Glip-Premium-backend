...
password = os.environ.get("GLIP_BOOTSTRAP_PASSWORD") or getpass.getpass(
    "GLIP native password: "
)
...
with SessionLocal() as db:
    ...
    if existing is not None:
        raise SystemExit(
            "refusing: native credential already exists for tenant/email"
        )

    tenant = db.get(Tenant, tenant_id)
    if tenant is None:
        db.add(Tenant(id=tenant_id, name=tenant_name))
        # Preserve a single transaction, but force the FK parent row to exist
        # before creating membership/credential children.
        db.flush()
    ...
    # NativeCredential has a database FK to memberships.id. Flush the
    # membership first so PostgreSQL cannot observe the child before its parent.
    db.flush()

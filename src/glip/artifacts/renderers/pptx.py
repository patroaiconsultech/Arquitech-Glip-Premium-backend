...
import io
from ..specs import ArtifactDocumentSpec,build_slide_plan
...
def render_pptx(spec:ArtifactDocumentSpec)->bytes:
    ...
    prs.core_properties.title=deck_title; prs.core_properties.author="GLIP"
    title_slide=prs.slides.add_slide(prs.slide_layouts[0]); title_slide.shapes.title.text=deck_title
    title_slide.placeholders[1].text="GLIP"
    ...
    for item in slides:
        ...
        footer.text_frame.text="GLIP"; footer.text_frame.paragraphs[0].alignment=PP_ALIGN.RIGHT; footer.text_frame.paragraphs[0].font.size=Pt(8)

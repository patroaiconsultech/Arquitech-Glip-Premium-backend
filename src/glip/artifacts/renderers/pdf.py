...
import io
from ..specs import ArtifactDocumentSpec,HeadingBlock,ParagraphBlock,ListBlock,TableBlock,PageBreakBlock
...
def render_pdf(spec:ArtifactDocumentSpec)->bytes:
    ...
    out=io.BytesIO()
    doc=SimpleDocTemplate(out,pagesize=A4,leftMargin=18*mm,rightMargin=18*mm,topMargin=22*mm,bottomMargin=20*mm,title=spec.title,author="GLIP")
    styles=getSampleStyleSheet()
    body=ParagraphStyle("GlipBody",parent=styles["BodyText"],fontName="Helvetica",fontSize=9.8,leading=13.2,spaceAfter=6)
    h1=ParagraphStyle("GlipH1",parent=styles["Heading1"],fontName="Helvetica-Bold",fontSize=18,leading=21,spaceAfter=8)
    h2=ParagraphStyle("GlipH2",parent=styles["Heading2"],fontName="Helvetica-Bold",fontSize=14,leading=17,spaceAfter=6)
    h3=ParagraphStyle("GlipH3",parent=styles["Heading3"],fontName="Helvetica-Bold",fontSize=11.5,leading=14,spaceAfter=4)
    story=[]
    ...
    def header_footer(canvas,built):
        ...
        canvas.drawString(doc.leftMargin,height-12*mm,"GLIP")
